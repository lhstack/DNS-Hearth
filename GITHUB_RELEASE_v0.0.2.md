...
```text
DNS Hearth v0.0.2 — Universal 2 Element Plus 界面与 DNS 管理
```
...
`v0.0.2` 是 DNS Hearth 的第二个预览版本，提供同时支持 Intel Mac 与 Apple Silicon 的 Universal 2 安装包，并重点改进 Element Plus 管理界面、页面布局、应用后台运行、Dock/托盘窗口恢复、系统 DNS 生命周期管理、首页自动应用、查询策略语义、缓存 TTL 说明和 DoH3 诊断。
...
- 应用过程中显示当前阶段、超时和错误状态；
- 支持取消等待中的自动应用操作；
- 修复切换或重新进入首页时，初始化配置被误判为用户修改并弹出“配置已自动应用”的问题。

### Element Plus 界面与布局修复

- 全局恢复为 Element Plus 默认设计语言：标准蓝色主操作、白色卡片、浅灰页面背景及标准成功、警告、危险状态色；
- 移除自定义暖色渐变、玻璃质感及大量覆盖组件默认行为的全局样式；
- 移除自定义按钮和输入框组合样式，业务页面统一使用 Element Plus 的 `el-button`、`el-input` 与图标组件；
- 修复侧边栏收起时菜单文字短暂残留、双重动画造成卡顿的问题，并简化折叠状态品牌图标；
- “清除指定域名缓存”改为独立的标准 `el-input` 与 `el-button`，避免 append 容器影响按钮可读性；
- 修复重写规则批量导入前端发送单个对象而后端要求数组的契约错误；
- 批量导入会按换行拆分域名，每行创建一条规则，并自动跳过空行、`#`/`!` 注释行及重复域名；
- 修复重写规则管理页一次性加载全部规则的问题，改为后端分页和后端聚合统计，避免十万条规则通过IPC一次性传输到前端；
- DNS重写引擎对精确匹配和标准`*.example.com`通配符建立内存索引，普通查询不再遍历全部精确规则；正则和复杂通配符仍保持按优先级匹配，避免改变现有规则语义；
- 修复查询策略“保存策略”按钮和 DNS 查询“查询”按钮文字看不清的问题；
- 修复首页“加入本地 DNS”文字按钮被全局样式覆盖成实心色块的问题；
- 已包含 `127.0.0.1` 时明确显示“已加入本地 DNS”；
- 修复切换页面后保留旧滚动位置，导致缓存页等页面标题和统计区域被顶部栏遮住的问题；
- 重新整理缓存页配置卡片、命中率图表和缓存操作区，避免说明换行、输入框和按钮挤压错位。
...
- 重写规则支持按行批量创建、文本文件导入和服务端分页管理，一行一个域名，自动跳过空行、注释行并去重；
- DNS重写引擎为精确域名和`*.example.com`通配符建立内存索引，避免每次查询扫描全部规则；正则和复杂通配符仍按优先级顺序匹配；
- 支持 UDP、DoT、DoH、DoQ 和 DoH3 上游协议；
- 支持并发、最快响应、轮询和随机查询策略；
- “并发查询”同时请求所有健康上游，首个 `NOERROR` 立即返回；若无 `NOERROR`，全部完成后返回最快 `NXDOMAIN`；
- “最快响应”根据历史成功查询延迟选择最低延迟的健康上游，只向该上游发送查询；
- 首次启动且没有保存过查询策略时默认使用“并发查询”；已有用户保存的策略保持不变；
- 提供 DNS 缓存管理、查询测试、查询日志和上游健康检查；
- 缓存遵守上游 Answer 记录中的最小 TTL；页面默认 TTL 仅用于没有 Answer TTL 的响应；
- 上游 TTL 较短的域名在 TTL 到期后重新查询属于正常行为；
- 单实例运行，避免多个应用实例争用本地 DNS 端口；
- 所有管理操作通过本地 Tauri IPC 完成，不开放 HTTP 管理端口；
- 使用统一的 Element Plus 管理界面，并修复首页、缓存页、查询策略和 DNS 查询页的按钮可读性、滚动与布局错位。
...
```text
版本：0.0.2
DMG 大小：21301604 bytes
DMG SHA-256：345080a160c0e076c453945f4a85178aba4c63ea2102b24cdbfc6baf61a7837d
应用主二进制 SHA-256：6eec5177634e8135e3918ad0f69392f8b74c7a2841c448413d8eec6494049866
